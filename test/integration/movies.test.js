...
describe('routes : movies', () => {
   ...
   describe('PUT /api/v1/movies/:id', () => {
      it('should return movie that was updated', (done) => {
          knex('movies')
            .select('*')
            .then(movie => {
      ...
      it('should throw an error if the movie does not exist', (done) => {
         chai.request(server) 
            ...
            .end((err, res) => {
               ...
               res.body.status.should.eql('error');
               // The JSON response body should have a
               // Key-value pair of { "message": "that movie doe not exist." }
               res.body.message.should.eql('That movie does not exist.');
               done();
            })
      })
       ...
       it('should throw an error if the movie does not exist', (done) => {
          chai.request(server)
            ...
            .end((err, res) => {
               ...
               res.body.status.should.eql('error');
               // The JSON response body should have a
               // Key-value pair of { "message": "That movie does not exisst" }
               res.body.message.should.eql('That movie does not exist.');
               done();
            })
       })
