...
const queries = require('../database/queries/movies');
...
router.get(BASE_URL, async (ctx) => {
    try {
       ...
       // ctx.status = 404;
       // ctx.message = "Resource not found";
       console.log(err);
...
router.put(`${BASE_URL}/:id`, async (ctx) => {
    try {
        const movie = await queries.updatedMovie(ctx.params.id, ctx.request.body);
        if (movie.length) {
            ctx.status = 200;
            ctx.body = {
                status: 'success',
...
router.delete(`${BASE_URL}/:id`, async (ctx) => {
   try {
       ...
       ctx.body = {
           status: 'error',
           message: err.message || 'Sorry, an error ocurred.'
       }
   }
})
